...
export default function AccordionTest() {
  ...
  const _renderHeader = item => {
    ...
    return (
      <View>
        <Text style={{backgroundColor: '#fdde1111'}}> {item.username[0]}</Text>
        ...
        <List style={styles.headMain}>
          ...
          <View style={{marginLeft: 9}}>
            <Text style={(styles.headTitle, {})}>{item.username}</Text>
